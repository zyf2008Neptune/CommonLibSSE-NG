...
namespace RE
{
	...
	{
		REL::Relocation<TaskQueueInterface**> singleton{ REL::RelocationID(517228, 403759) };
		return *singleton;
	}
	...
	std::uint32_t TaskQueueInterface::QueueAddRipple(float a_scale, const NiPoint3& a_pos)
	{
		...
		REL::Relocation<func_t> func{ REL::RelocationID(35978, 36953) };
	...
	std::uint32_t TaskQueueInterface::QueueRemoveSpell(RE::ActorHandle& a_actorHandle, RE::SpellItem* a_spellItem)
	{
		...
		REL::Relocation<func_t> func{ REL::RelocationID(35987, 36962) };
